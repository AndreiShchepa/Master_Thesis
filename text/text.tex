...
TODO about it reuires alot of harsare and it is very expensive, alot of data and othe drawbacks, that make this way very inefecient in todays reality.


\subsection{Fine-tuning}
\label{subsec:finetuning}

Fine-tuning adapts a pre-trained model to specific downstream tasks by continuing training on a smaller, curated dataset.  The most common modern approach is supervised fine-tuning on instruction–response pairs, which teaches the model to follow user intent reliably.
...
An alternative gaining traction is \textbf{Direct Preference Optimization (DPO)}~\cite{rafailov2024}, which eliminates the separate reward model by directly optimising the language model on preference pairs.  Smart-LLaMA-DPO~\cite{yu2025smart} uses this approach for smart contract vulnerability detection, applying DPO on paired high-quality and low-quality vulnerability explanations to improve both detection accuracy and explanation quality.

[TODO tell more about it]

\subsection{Prompting}
\label{subsec:prompting}

\emph{Prompting} shapes model behavior at inference time through instructions, context, and examples without changing any model weights.  Brown et al.~\cite{brown2020languagemodelsfewshotlearners} demonstrated that including a few input–output examples (\emph{few-shot prompting}) can elicit task-specific behaviour from large decoder-only models without any fine-tuning at all. [TODO it opens for us also different methods, how use all benefits of prompting emgineering.]

\textbf{Task decomposition} breaks a complex task into smaller sub-tasks. Each sub-task has narrower scope and clearer success criteria. [TODO smth]

\textbf{Chain-of-thought (CoT)} prompting~\cite{wei2023chain} elicits intermediate reasoning steps and improves performance on multi-step tasks.  In security settings, however, the goal is not free-form reasoning traces but \emph{checkable} reasoning: claims should be tied to specific code locations or retrieved evidence.  ReVul-CoT~\cite{chen2025revulcot} demonstrates this principle by guiding LLMs through structured step-by-step CVSS assessment grounded in retrieved vulnerability records.
...
% ============================================================
What common is rom all approaches for LLM? It treats a text as numbers. [ TODO Start talk about embeddings]
