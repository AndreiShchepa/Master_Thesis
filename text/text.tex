...
\begin{minted}[fontsize=\small,breaklines]{solidity}function susceptible to reentrancy.}, label=lst:reentrancy_bad]
...
\begin{lstlisting}[language=Solidity, caption={Attacker contract exploiting the reentrancy vulnerability.}, label=lst:reentrancy_attacker]
contract Attacker {
...
The fix is straightforward: apply the \emph{checks-effects-interactions} (CEI) pattern---update all internal state \emph{before} making any external call. An additional defense is an explicit reentrancy guard.

\begin{lstlisting}[language=Solidity, caption={Secure withdrawal following the checks-effects-interactions pattern.}, label=lst:reentrancy_good]
// SECURE
contract SecureVault {
    ...
    function withdraw() external nonReentrant {
        ...
        // (1) Effects: state updated BEFORE the call
        balances[msg.sender] = 0;

        // (2) Interaction: external call is now safe
        (bool ok, ) = msg.sender.call{value: amount}("");
        require(ok, "Transfer failed");
    }
}
...
\begin{lstlisting}[language=Solidity, caption={Access control vulnerability: unprotected initializer exposed as a regular function.}, label=lst:access_bad]
// VULNERABLE (pre-Solidity 0.4.22 pattern)
contract OwnedWallet {
    address public owner;

    // Intended as constructor, but name mismatch makes it
    // a public function callable by anyone at any time
    function OwnedWallt() public {   // typo: 'Wallt' != 'OwnedWallet'
        owner = msg.sender;
    }
...
Any caller can invoke \texttt{OwnedWallt()} to claim ownership, then drain the wallet. The modern fix is to use the \texttt{constructor} keyword (mandatory since Solidity 0.4.22) and to use role-based access control patterns from audited libraries.

\begin{lstlisting}[language=Solidity, caption={Secure access control using the \texttt{constructor} keyword and an \texttt{onlyOwner} modifier.}, label=lst:access_good]
// SECURE
contract OwnedWallet {
    address public owner;
...
\paragraph{Real-world impact: Parity MultiSig Hack (2017).}
...
\begin{lstlisting}[language=Solidity, caption={DoS via push-payment to an array of recipients: one reverting recipient blocks all others.}, label=lst:dos_bad]
// VULNERABLE: push-payment pattern over unbounded array
...
\begin{lstlisting}[language=Solidity, caption={Secure pull-payment pattern: each recipient claims their own funds.}, label=lst:dos_good]
// SECURE: pull-payment
contract PullDistributor {
    mapping(address => uint256) public owed;

    function credit(address r, uint256 amount) external {
        owed[r] += amount;
    }

    // Each user withdraws independently -- one failure
...
\begin{lstlisting}[language=Solidity, caption={Vulnerable DEX interaction with no slippage protection, exposing the user to sandwich attacks.}, label=lst:frontrun_bad]
// VULNERABLE: no minimum output specified
function swapETHForToken(address token) external payable {
    uint256 amountOut = dex.getAmountOut(msg.value, token);
    // Attacker front-runs to inflate price, so actual
    // amountOut at execution is much less than quoted
    dex.swap{value: msg.value}(token, 0); // minOut = 0
}
\end{lstlisting}

\begin{lstlisting}[language=Solidity, caption={Mitigated swap with explicit slippage bound and deadline.}, label=lst:frontrun_good]
// SECURE: caller specifies acceptable minimum and deadline
function swapETHForToken(
    ...
    dex.swap{value: msg.value}(token, minAmountOut);
    // Reverts if price has moved beyond the user's tolerance
}
\end{lstlisting}
